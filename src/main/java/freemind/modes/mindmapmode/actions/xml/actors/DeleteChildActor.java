...
public class DeleteChildActor extends XmlActorAdapter {
	...
	public void deleteWithoutUndo(MindMapNode selectedNode) {
		...
		// remove hooks:
		removeHooks((NodeAdapter) selectedNode);
		...
		if(view != null) {
			...
			if(view.getSelecteds().size() == 0) {
				...
				int childIndex = parent.getChildPosition(selectedNode).get();
				if(parent.childCount() > childIndex+1) {
					// the next node
					newSelectedView = view.getNodeView((MindMapNode) parent.childAt(childIndex+1));
	...
	public void deleteNode(MindMapNode selectedNode) {
		...
		((NodeAdapter) selectedNode).isLeft());
		// Undo-action
