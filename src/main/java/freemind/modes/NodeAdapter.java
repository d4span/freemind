...
public abstract class NodeAdapter implements MindMapNode {
    ...
    protected NodeAdapter(Object userObject, MindMap pMap) {
        ...
        if (logger == null) logger = Resources.getInstance()
        ...
        if (sSaveIdPropertyChangeListener == null) {
            ...
            Controller.addPropertyChangeListenerAndPropagate(
    ...
    public String getShortText(ModeController controller) {
        ...
        if (adaptedText.length() > 40) adaptedText = adaptedText.substring(
            0,
            40
        ) +
        " ...";
    ...
    public void setLink(String link) {
        if (link != null && link.startsWith("#")) {
            getMap()
                .getLinkRegistry()
                .registerLocalHyperlinkId(link.substring(1));
        }
        this.link = link;
    }
    ...
    public String getStyle() {
        ...
        if (style == null) {
            if (this.isRoot()) {
                returnedString = getMapFeedback()
                    .getProperty(FreeMind.RESOURCES_ROOT_NODE_STYLE);
            } else {
                String stdstyle = getMapFeedback()
                    .getProperty(FreeMind.RESOURCES_NODE_STYLE);
    ...
    public void toggleStrikethrough() {
        ...
        if (
            attributes.containsKey(TextAttribute.STRIKETHROUGH) &&
    ...
    public void toggleBold() {
        ...
        setFont(
            getMapFeedback()
                .getFontThroughMap(font.deriveFont(font.getStyle() ^ Font.BOLD))
        );
    }
    ...
    public void toggleItalic() {
        ...
        setFont(
            getMapFeedback()
                .getFontThroughMap(
                    font.deriveFont(font.getStyle() ^ Font.ITALIC)
                )
        );
    }
    ...
    public void setFontSize(int fontSize) {
        ...
        setFont(
            getMapFeedback()
                .getFontThroughMap(font.deriveFont((float) fontSize))
        );
    }
    ...
    public boolean isStrikethrough() {
        if (font != null) {
            ...
            if (attr.containsKey(TextAttribute.STRIKETHROUGH)) {
                return (
                    attr.get(TextAttribute.STRIKETHROUGH) ==
                    TextAttribute.STRIKETHROUGH_ON
                );
            }
        }
    ...
    public MindMapNode shallowCopy() {
        try {
            ...
            MindMapNode copy = getMap()
                .createNodeTreeFromXml(new StringReader(result), IDToTarget);
    ...
    public boolean isDescendantOf(MindMapNode pParentNode) {
        ...
        else if (pParentNode == getParentNode()) return true;
        else return getParentNode().isDescendantOf(pParentNode);
    }
    ...
    public int getChildPosition(MindMapNode childNode) {
        int position = 0;
        for (
            ListIterator<MindMapNode> i = children.listIterator();
            i.hasNext();
            ++position
        ) {
            if (i.next() == childNode) {
    ...
    public List<MindMapNode> getChildren() {
        return Collections.unmodifiableList(
            (children != null) ? children : Collections.<MindMapNode>emptyList()
        );
    }
    ...
    @Override
    public MutableTreeNode<String> getParent() {
        return parent;
    }
    ...
    public boolean isNewChildLeft() {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            if (!((MindMapNode) get(i)).isLeft()) rightChildrenCount++;
        ...
        if (node == this.preferredChild) { // mind preferred child :-) (PN)
            ...
            if (children.size() > index + 1) {
                ...
                this.preferredChild = (index > 0)
                    ? (MindMapNode) (children.get(index - 1))
                    : null;
            }
        }
        node.setParent(null);
    ...
    private void recursiveCallAddChildren(
        MindMapNode node,
        ...
        if (
            !node.isRoot() && node.getParentNode() != null
     ...
     */
    private void recursiveCallRemoveChildren(
        MindMapNode node,
        MindMapNode removedChild,
        MindMapNode oldDad
    ) {
    ...
    public PermanentNodeHook addHook(PermanentNodeHook hook) {
        // add then
        if (hook == null) throw new IllegalArgumentException(
            "Added null hook."
        );
        createHooks();
        hooks.add(hook);
    ...
    public SortedMap<String, String> getToolTip() {
        ...
        if (result == null) result = new TreeMap<>();
        ...
        if (
            link != null &&
            link.endsWith(FreeMindCommon.FREEMIND_FILE_EXTENSION)
        ) {
    ...
    public XMLElement save(
        ...
        if (!HtmlTools.isHtmlNode(text)) {
            ...
            htmlElement.setAttribute(
                XMLElementAdapter.XML_NODE_XHTML_TYPE_TAG,
                XMLElementAdapter.XML_NODE_XHTML_TYPE_NODE
            );
            htmlElement.setEncodedContent(
                convertToEncodedContent(getXmlText())
            );
        ...
        if (getXmlNoteText() != null) {
            ...
            htmlElement.setEncodedContent(
                convertToEncodedContent(getXmlNoteText())
            );
            node.addChild(htmlElement);
        }
        ...
        if (getAdditionalInfo() != null) {
            node.setAttribute(
                XMLElementAdapter.XML_NODE_ENCRYPTED_CONTENT,
                getAdditionalInfo()
            );
        }
        ...
        for (int i = 0; i < linkVector.size(); ++i) {
            if (linkVector.get(i) instanceof ArrowLinkAdapter) {
                XMLElement arrowLinkElement =
                    ((ArrowLinkAdapter) linkVector.get(i)).save();
        ...
        for (int i = 0; i < targetVector.size(); ++i) {
            if (targetVector.get(i) instanceof ArrowLinkAdapter) {
                XMLElement arrowLinkTargetElement =
                    ((ArrowLinkAdapter) targetVector.get(
                            i
                        )).createArrowLinkTarget(registry).save();
        ...
        if (
            !sSaveOnlyIntrinsicallyNeededIds ||
            (registry.isTargetOfLocalHyperlinks(label) ||
                (registry.getAllLinksIntoMe(this).size() > 0))
        ) {
            if (label != null) {
        ...
        if (historyInformation != null) {
            node.setAttribute(
                XMLElementAdapter.XML_NODE_HISTORY_CREATED_AT,
                Tools.dateToString(getHistoryInformation().getCreatedAt())
            );
            node.setAttribute(
                XMLElementAdapter.XML_NODE_HISTORY_LAST_MODIFIED_AT,
                Tools.dateToString(getHistoryInformation().getLastModifiedAt())
            );
        }
        // font
        if (font != null) {
    ...
    private void saveChildren(
        ...
        for (
            ListIterator<NodeAdapter> e = node.childrenUnfolded();
            e.hasNext();
        ) {
            NodeAdapter child = e.next();
            if (saveHidden || child.isVisible()) child.save(
                writer,
                registry,
                saveHidden,
                true
            );
            else saveChildren(writer, registry, child, saveHidden);
        }
    }
    ...
    public Map<String, ImageIcon> getStateIcons() {
        if (stateIcons == null) return Collections.emptyMap();
    ...
    public void checkAttributePosition(int pPosition) {
        if (
            mAttributeVector == null ||
            getAttributeTableLength() <= pPosition ||
            pPosition < 0
        ) {
            throw new IllegalArgumentException(
                "Attribute position out of range: " + pPosition
            );
        }
    }
