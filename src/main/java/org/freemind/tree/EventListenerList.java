...
public class EventListenerList implements Serializable {
    ...
    public <T extends EventListener> T[] getListeners(Class<T> t) {
       ...
       T[] result = (EventListener[])Array.newInstance(t, n);
       int j = 0;
 
       for(int i = lList.length - 2; i >= 0; i -= 2) {
          if (lList[i] == t) {
             T tmp = (EventListener)lList[i + 1];
             result[j++] = tmp;
          }
       }
    ...
    public synchronized <T extends EventListener> void add(Class<T> t, T l) {
       if (l != null) {
          if (!t.isInstance(l)) {
             ...
             throw new IllegalArgumentException("Listener " + var10002 + " is not of type " + String.valueOf(t));
          } else {
